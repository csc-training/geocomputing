...
In CSC's Roihu the current lidR version is 2.1.4 and can be loaded with `module load r-env` along with many other R packages and GIS software.
```{r results='hide' }
library("lidR")
```
...
In the same way you can read any LAS or LAZ files. You can find the lidar data provided by the National Land Survey of Finland/Maanmittauslaitos (NLSF/MML) ready to use in Roihu. For more information about this dataset (and other GIS datasets) see: [GIS data in Roihu](https://research.csc.fi/gis_data_in_csc_computing_env).

Once you know the path to a .las or .laz file in Roihu, you can open it with (note that the files are often over 100 Mb in size and will take a bit to load):
```{r  }
LASfile <- "/dataset/project_2019680/mml/laserkeilaus/2008_latest/2019/L331/1/L3313F3.laz"
las <- readLAS(LASfile)
print(las)
```
...
## Creating a Digital Terrain Model as an array job in Roihu
...
Let's use the `grid_terrain` function to run an R script that we can run as an array job in Roihu. An array job is meant to run the same script on multiple files given for ex. as a list of files in a text file.

Array jobs need a list of filepaths to the LAZ files. Example of this kind of file is the file las_files.txt

Open the file `simple_lidR.R` and study it. It includes some parts that relate to the batch job you will need to send a request to Roihu.

Open also the Roihu batch job file `simple_lidR_arrayjob.sh`, it includes some explanations on the contents.

Once you are sure all the parameters are correctly set, **open a terminal connection to Roihu** (using Putty or you can also open it from NoMachine). Go to the project folder and send your batch job with:
```bash
