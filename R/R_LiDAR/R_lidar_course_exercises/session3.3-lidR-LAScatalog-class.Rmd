...
```{r}
ctg_subset <- catalog("/dataset/project_2019680/mml/laserkeilaus/2008_latest/2019/U442/1/")
print(ctg_subset)
...
## Creating a Digital Terrain Model as a parallel job in Roihu
...
Open the file `simple_catalog_lidR.R` and study it. It includes some parts that relate to the batch job you will need to send a request to Roihu.

Open also the batch job file `simple_catalog_lidR_batchjob.sh`, it includes some explanations on the contents.

Once you are sure all the parameters are correctly set, open a terminal to Roihu (using Putty or you can also open it from NoMachine). Go to the project folder and send your batch job with:
```bash
