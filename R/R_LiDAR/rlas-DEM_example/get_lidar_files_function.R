# Function to get LiDAR files from Roihu's GIS datasets for a
# given area of interest

# Make sure that you have Roihu's R spatial environment loaded by using module load r-env or by using interactively 
# GIS RStudio in NoMachine.

lidar_files_roihu <- function(f_poly){
  ...
  # The LiDAR dataset in Roihu is located at /dataset/project_2019680/mml/laserkeilaus/2008_latest/2008_latest.shp
  # You should use the lidar_auto_all.shp index file to spatially look for LiDAR files
  f_lidar_index <- "/dataset/project_2019680/mml/laserkeilaus/2008_latest/2008_latest.shp"
  ...
  for (i in 1:length(inters@polygons)){
    file <- paste0("/dataset/project_2019680/",inters$path[i])
    files <- c(files, file)
  }
  print(files)
  return(files)
...
# lidar_files <- lidar_files_roihu("./area_of_interest.shp")
# print(lidar_files)
