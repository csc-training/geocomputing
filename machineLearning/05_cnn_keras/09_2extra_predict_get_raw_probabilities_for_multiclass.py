...
prediction_image_file = os.path.join(results_dir,'predicted_'+model_name+'.tif')
prediction_vrt_file = os.path.join(results_dir,'predicted_'+model_name+'.vrt')
...
def predictTile(model, dataImage):
    ...
    with rasterio.open(dataImage, 'r') as image_dataset:    
        ...
        prediction = prediction.reshape(img_size, img_size, no_of_classes)     
        
        prediction2 = np.transpose(prediction, (2, 0, 1))
        print(prediction.shape)
        dtype='float32'
        ...
        outputMeta.update(count=1, dtype=dtype)
        outputMeta.update(count=no_of_classes, dtype=dtype)
...
#tensorflow.keras.losses.jaccard_loss = jaccard_loss
