...
if no_of_classes == 2: 
    ...
    model_best = os.path.join(results_dir, 'model_best_multiclass_05_001_2003.h5')
    training_log_file = os.path.join(results_dir, 'log_multiclass_05_001_2003.csv')    
...
def data_gen(img_df, augment):
  ...
  while (True):
    ...
    mask = np.zeros((batch_size, crop_size, crop_size)).astype('float') #, no_of_classes
