...
def trainModel(x_train, y_train):
	...
	# - the activation is 'softmax', suitable for multi-class classification models
	# - size=4, for 4 classes
    model.add(layers.Dense(4, activation='softmax'))
    
	# Compile the model, using:
	# - Adam optimizer, often used, but could be some other too
	# -- learning_rate is 0.01. you may change it but be careful since overshooting may happen
...
def estimateModel(trained_model, x_test, y_test):
    ...
    print ('Text prediction dataframe shape, original 2D: ', test_prediction.shape) 	
    ...
    print ('Text prediction dataframe shape, after argmax, 1D: ', test_prediction.shape) 	
...
def predictImage(predictedImagePath, predictImage):
    ...
    with rasterio.open(predictImage, 'r') as image_dataset:
        ...
        print ('Prediction dataframe shape, after argmax, 1D: ', test_prediction.shape) 	
...
def main():
    ...
    # Initialize the fully connected classifier and give the hyperparameters.
